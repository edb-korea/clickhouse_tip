...
구성:
  - deck.gl GeoJSON   : 시군구 polygon + 관측소 point 단일 지도 (station_geojson_overlay)
...
URI = "clickhousedb://superset_user:data12!%40@localhost:8123/geospatial"
...
def get_db_id(token, csrf):
    db_id = find_id("database", "database_name", DB_NAME, token, csrf)
    if db_id:
        return db_id
    ...
    if st not in (200, 201):
        raise RuntimeError(f"database create failed: status={st}, response={j}")
    return j["id"]
...
def main():
    ...
    ds_overlay = ensure_dataset(token, csrf, db_id, "station_geojson_overlay")

    # 1) deck.gl Scatter — 관측소 포인트 (단독 확인용)
    ...
    # 2) deck.gl Polygon — 시군구별 관측소 수 choropleth (단독 확인용)
    ...
    # 3) deck.gl GeoJSON — polygon + point를 한 chart 안에서 렌더링
    overlay = ensure_chart(
        token, csrf, "전국 관측소 + 시군구 집계 오버레이 (deck.gl GeoJSON)", "deck_geojson", ds_overlay, {
            "geojson": "feature", "adhoc_filters": [], "row_limit": 10000,
            "viewport": VIEWPORT, "autozoom": True, "mapbox_style": MAP_STYLE,
            "filled": True, "stroked": True, "extruded": False,
            "line_width": 1,
            "color_picker": {"r": 220, "g": 30, "b": 30, "a": 1},
            "fill_color_picker": {"r": 3, "g": 165, "b": 150, "a": 0.45},
            "stroke_color_picker": {"r": 255, "g": 255, "b": 255, "a": 1},
            "point_radius_fixed": {"type": "fix", "value": 2500},
        })

    # 4) deck.gl Multi — 기존 방식의 오버레이. dashboard에는 쓰지 않고 단독 확인용으로만 만든다.
    multi = ensure_chart(
    ...
    # 6) 대시보드 (position_json 으로 레이아웃 배치)
    ...
    pos = {
        ...
        "HEADER_ID": {"type": "HEADER", "id": "HEADER_ID",
                      "meta": {"text": "전국 관측소 현황"}},
        "ROW-A": {"type": "ROW", "id": "ROW-A", "children": ["CHART-overlay"],
                  "parents": ["ROOT_ID", "GRID_ID"],
                  "meta": {"background": "BACKGROUND_TRANSPARENT"}},
        "CHART-overlay": chart_node("CHART-overlay", overlay,
    ...
    for cid in (overlay, polygon, table):
        req("PUT", f"/api/v1/chart/{cid}", token, csrf, {"dashboards": [did]})
    print(f"[dash] linked charts {overlay},{polygon},{table} -> dashboard {did}")

    print(f"\n[dash] 완료 → http://localhost:8088/superset/dashboard/{did}/")
    print(f"[dash] GeoJSON 오버레이 지도 단독: http://localhost:8088/explore/?slice_id={overlay}")
    print(f"[dash] 기존 Multi 오버레이 지도 단독: http://localhost:8088/explore/?slice_id={multi}")
